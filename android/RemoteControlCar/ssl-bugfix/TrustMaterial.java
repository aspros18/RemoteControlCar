/*
 * $HeadURL: http://juliusdavies.ca/svn/not-yet-commons-ssl/tags/commons-ssl-0.3.11/src/java/org/apache/commons/ssl/TrustMaterial.java $
 * $Revision: 148 $
 * $Date: 2009-05-25 23:07:21 -0700 (Mon, 25 May 2009) $
 *
 * ====================================================================
...
public class TrustMaterial extends TrustChain {
    ...
    TrustMaterial(KeyStore jks, int simpleTrustType) {
        if (jks == null && simpleTrustType != 0) {
        	...
        	KeyStore empty = null;
			try {
				empty = KeyStore.getInstance(KeyStore.getDefaultType());
				empty.load(null, new char[]{});
			} catch (Exception e) {
				;
			}
			...
			// If JSSE_CACERTS and CACERTS are null, the empty KeyStore will be used.
        	this.jks = CACERTS != null ? CACERTS.jks : JSSE_CACERTS == null ? empty : JSSE_CACERTS.jks;
        } else {
            this.jks = jks;
        }
